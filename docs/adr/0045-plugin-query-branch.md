...
- `MAX_SOURCES` (10) is now shared across SQL, REST and plugin sources, sorted alphabetically by name — a plugin source is cheap to accumulate (no introspection call, so it never drops out the way an unreadable SQL source does), so an org with many plugin connectors could crowd SQL sources out of the prompt before a build that actually needed one of them ever sees it. Not fixed in this pass; worth knowing if a future report is "my connected SQL source isn't showing up."

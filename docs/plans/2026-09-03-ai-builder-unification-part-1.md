...
> `UpdateTable` закрыт в engine `STEP_TYPES`). Часть 2 (`docs/plans/` — план ещё не создан)
> берёт на себя каталог движка 11→36, деплой, удаление fallback, фронтенд.
...
### Task 3: ADR-0052 — hard switch, отмена silent fallback

...
- [ ] Захват `usage` из AI SDK (`AIGatewayGenerate`, стрим-пути) → `metadata.usage` ai-сообщения (jsonb, без миграций).
- [ ] Эндпоинт `GET /ai/conversation/:id/token-usage` (сумма + breakdown), реализовать `getThreadTokenUsage`.
- [ ] Фронт: индикатор расхода в футере чат-панели.

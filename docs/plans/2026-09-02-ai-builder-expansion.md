...
- [ ] Планировщик разрешает новые типы шагов; ADR-0027 «diff-merge editing and lifecycle». Планировщик уже обновлён под `DeleteComponent`/`DeleteQuery` (промпт + `STEP_TYPES`/`SUPPORTED_STEP_TYPES`, коммит `797099f69f`); ADR-0027 ещё не написан — оформить после того, как определится модель nesting выше (ADR должен описывать оба).

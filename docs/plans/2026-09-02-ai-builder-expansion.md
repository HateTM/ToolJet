...
- [x] ADR «diff-merge editing and lifecycle» — номер 0027 уже занят (дважды, генерацией-engine и token-usage), оформлено как **ADR-0043** (`docs/adr/0043-component-lifecycle-and-create-time-nesting.md`): описывает и общий Update/Delete lifecycle (grounding → sparse diff → undo-snapshot), и решение по nesting из этого прохода (create-time only, Container/Form only, slot-контейнеры — Tabs/Listview/ModalV2 — сознательно вне скоупа).

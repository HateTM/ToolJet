...
describe('resolveGeneratedStepArgs (ADR-0048 follow-up: deterministic consumption)', () => {
  it('returns the payload when it has the required args for the step type', () => {
    ...
    expect(
      resolveGeneratedStepArgs(stepWith('UpdateComponent', { generatedStep: payload })),
    ).toEqual(payload);
  });
  ...
  it('returns null when props or generatedStep is missing or not a plain object', () => {
    ...
    expect(
      resolveGeneratedStepArgs(stepWith('UpdateComponent', { generatedStep: 'updateComponent' })),
    ).toBeNull();
    expect(
  ...
  it('returns null when a required arg is missing or of the wrong kind', () => {
    ...
    expect(
      resolveGeneratedStepArgs(stepWith('DeleteComponent', { generatedStep: { componentId: 42 } })),
    ).toBeNull();
    // UpdateQuery requires queryName: string + options: object
    expect(
      resolveGeneratedStepArgs(stepWith('UpdateQuery', { generatedStep: { queryName: 'q1' } })),
    ).toBeNull();
    expect(
      resolveGeneratedStepArgs(
        stepWith('UpdateQuery', { generatedStep: { queryName: 'q1', options: [1, 2] } }),
      ),
    ).toBeNull();
  });
  ...
  it('returns null for step types the engine never produces payloads for', () => {
    expect(
      resolveGeneratedStepArgs(stepWith('CreateTable', { generatedStep: { table_name: 't' } })),
    ).toBeNull();
    expect(
      resolveGeneratedStepArgs(stepWith('UpdateTable', { generatedStep: { table_name: 't' } })),
    ).toBeNull();
  });
});
